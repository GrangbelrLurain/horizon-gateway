...
/// Emit to GUI webview: serve event bus (when headless) and/or in-process Tauri emit.
pub fn emit_to_gui<S: Serialize + Clone>(app: Option<&tauri::AppHandle>, event: &str, payload: S) {
    publish_event(event, payload.clone());
    if let Some(app) = app {
