...
- 모니터링 중인 특정 웹페이지 도메인에 Watchtower의 디버깅 Inspector 스크립트를 동적으로 주입할 수 있습니다.
- 이를 통해 클라이언트 브라우저의 콘솔 로그, 네트워크 요청, DOM 트리 상태 등을 Watchtower 앱 내에서 실시간으로 원격 검사하고 제어할 수 있습니다.
...
Watchtower는 GUI 클라이언트 환경 외에도 강력한 명령행 인터페이스(CLI)를 갖추고 있어 터미널을 통한 설정 제어와 AI 코딩 에이전트(Cursor, Gemini, Claude 등)와의 자동화 연동을 지원합니다.
...
AI 어시스턴트가 프로젝트 내에서 Watchtower 기능을 자동 인지하고 제어할 수 있도록 스킬 문서를 설치합니다.
- **프로젝트 로컬 설치**:
  ```bash
  watchtower cli init --project
  ```
  현재 프로젝트의 `.agents/skills/watchtower` 경로에 `SKILL.md` 및 관련 헬퍼 스크립트를 생성합니다.
- **글로벌 에이전트 설치**:
  ```bash
...
CLI 모드에서 호출 및 제어 가능한 모든 Watchtower 내부 API 명령어 목록을 JSON 양식으로 출력합니다.
```bash
watchtower cli list
```
...
#### 4. `run` (API 명령어 실행)
Watchtower의 백엔드 비즈니스 로직을 직접 실행합니다. 두 번째 인자로 JSON 페이로드를 전달하며, `--query` 옵션을 통해 출력 결과를 필터링(jq 형태의 쿼리)할 수 있습니다.
- **전체 도메인 조회**:
  ```bash
...
- **특정 그룹 생성**:
  ```bash
  watchtower cli run create_group '{"name": "운영 도메인"}'
  ```
- **결과 필터링 예시**:
  ```bash
